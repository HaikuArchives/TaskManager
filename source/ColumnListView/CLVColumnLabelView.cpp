	...
	for(int32 ColumnDraw = 0; ColumnDraw < NumberOfColumns; ColumnDraw++)
	{
		...
		if(ThisColumn->IsShown())
		{
				...
				ViewBounds.bottom)))
			{
				...
				if(fParent->fSortKeyList.ItemAt(0) == ThisColumn)
					primary_sort_key = true;
				else
					primary_sort_key = false;
				...
				if(MergeWithRight && !(ThisColumn == fColumnClicked && fColumnResizing))
					Stop.x = ColumnEnd;
				AddLine(Start,Stop,CColor::BeHighlight);
				//Left line
				if(!MergeWithLeft)
					AddLine(BPoint(ColumnBegin,ViewBounds.top+1.0),
						BPoint(ColumnBegin,ViewBounds.bottom),CColor::BeHighlight);
				//Bottom line
				...
				if(MergeWithRight && !(ThisColumn == fColumnClicked && fColumnResizing))
					Stop.x = ColumnEnd;
				AddLine(Start,Stop,CColor::BeShadow);
				//Right line
				if(ThisColumn == fColumnClicked && fColumnResizing)
				...
				// ADDED BY TK 04Feb2000
				
				// Triangle
				if( sort_key && !(ThisColumn->fFlags & CLV_NOT_SHOW_ARROW))
				{			
					if( ThisColumn->SortMode() == Ascending )
					{
						BPoint p1(ColumnEnd-2.0-fFontAscent, ViewBounds.top+4.0);
						BPoint p2(ColumnEnd-2.0-fFontAscent/2, ViewBounds.top+4.0+fFontAscent);
						BPoint p3(ColumnEnd-2.0, ViewBounds.top+4.0);
						
						AddLine(p1, p3, CColor::BeShadow);
						AddLine(p1, p2, CColor::BeShadow);
						AddLine(p3, p2, CColor::BeHighlight);
					} 
					else if( ThisColumn->SortMode() == Descending ) 
					{
						BPoint p1(ColumnEnd-2.0-fFontAscent, ViewBounds.top+4.0+fFontAscent);
						BPoint p2(ColumnEnd-2.0-fFontAscent/2, ViewBounds.top+4.0);
						BPoint p3(ColumnEnd-2.0, ViewBounds.top+4.0+fFontAscent);
				...
				// ADDED BY TK 04Feb2000
				if( sort_key && !(ThisColumn->fFlags & CLV_NOT_SHOW_ARROW))
					TextRect = BRect(ColumnBegin+1.0,ViewBounds.top+1.0,ColumnEnd-3.0-fFontAscent,
										ViewBounds.bottom-1.0);
				else
					TextRect = BRect(ColumnBegin+1.0,ViewBounds.top+1.0,ColumnEnd-1.0,
										ViewBounds.bottom-1.0);
				...
				ThisColumn->DrawColumnHeader(this,TextRect,sort_key,primary_sort_key,focus,fFontAscent);
...
void CLVColumnLabelView::MouseMoved(BPoint Point, uint32 transit, const BMessage *message)
{
	BCursor vResizeCursor((const void *)Mak_Hcursor);
	...
	if(!fColumnResizing)
	{
		...
		for(int32 ColumnFind = 0; ColumnFind < NumberOfColumns; ColumnFind++)
		{
			...
			if(ThisColumn->IsShown())
			{
				...
				if(Point.x >= ColumnEnd-2.0 && Point.x <= ColumnEnd+3.0 && 
				   !(ThisColumn->fFlags & CLV_NOT_RESIZABLE))
				{
					SetViewCursor(&vResizeCursor);
					return;
				}
			}
		}
	...
	if(Buttons == B_PRIMARY_MOUSE_BUTTON)
	{
		...
		if(Window()->CurrentMessage()->FindInt32("clicks", &clicks) == B_OK) {
			if(clicks >= 2) {
				...
				for(int ColumnFind=0 ; ColumnFind<NumberOfColumns ; ColumnFind++) {
					CLVColumn *ThisColumn = (CLVColumn*)fDisplayList->ItemAt(ColumnFind);
					...
					if(ThisColumn->IsShown()) {
						...
						ColumnListView *parent = ThisColumn->fParent;
						BFont parentFont;
						
						parent->GetFont(&parentFont);
				
						if(MousePos.x >= ColumnEnd-2.0 && MousePos.x <= ColumnEnd+2.0) {
							// double click on column separator

							// change width of column so that the context fits in.
							
							int32 items = parent->CountItems();
							
							float maxWidth=0.0;
							
							for(int i=0 ; i<items ; i++) {
								CLVEasyItem *item = dynamic_cast<CLVEasyItem *>(parent->ItemAt(i));

								if(item) {							
									maxWidth = 
										MAX(item->GetColumnContentWidth(parent, &parentFont, ColumnFind), maxWidth);
								}
							}
							
							if(maxWidth > ColumnEnd - ColumnBegin) {
								ThisColumn->SetWidth(maxWidth+4.0);
							}
