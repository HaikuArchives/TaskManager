...
class CPreferences
{
	...
	void Read(const char *name, BString &value, const BString &defaultValue="");
	void Read(const char *name, rgb_color &value, const rgb_color &defaultValue);

	bool Write(const char *name, const char *buffer, int32 length=-1);
	...
	bool Write(const char *name, const BString &value);
	bool Write(const char *name, rgb_color value);
	
	virtual bool IsSpecified(const char *name) = 0;
...
class CFilePreferences : public CPreferences
{
	...
	virtual bool IsSpecified(const char *name);
